...
{
    /// <summary>
    /// OData Json serializer for resources and resource sets.
    /// </summary>
    internal sealed class ODataJsonResourceSerializer : ODataJsonPropertySerializer
    {
        /// <summary>
        /// Constructor.
        /// </summary>
        ...
        private Uri MetadataDocumentBaseUri
        {
            get
            {
                ...
                return this.JsonOutputContext.MessageWriterSettings.MetadataDocumentUri;
            }
        }
        ...
        internal void WriteResourceSetStartMetadataProperties(ODataResourceSet resourceSet, string propertyName, string expectedResourceTypeName, bool isUndeclared)
        {
            ...
            string typeName = this.JsonOutputContext.TypeNameOracle.GetResourceSetTypeNameForWriting(
        ...
        internal void WriteResourceStartMetadataProperties(IODataJsonWriterResourceState resourceState)
        {
            ...
            // Write the "@odata.type": "typename"
            string typeName = this.JsonOutputContext.TypeNameOracle.GetResourceTypeNameForWriting(
                expectedResourceTypeName,
                resource,
        ...
        internal void WriteResourceEndMetadataProperties(IODataJsonWriterResourceState resourceState, IDuplicatePropertyNameChecker duplicatePropertyNameChecker)
        {
            ...
            ODataJsonReaderNestedResourceInfo navigationLinkInfo = resource.MetadataBuilder.GetNextUnprocessedNavigationLink();
        ...
        internal void WriteOperations(IEnumerable<ODataOperation> operations, bool isAction)
        {
            ...
            IEnumerable<IGrouping<string, ODataOperation>> metadataGroups = operations.GroupBy(o =>
            {
                ...
                WriterValidationUtils.ValidateCanWriteOperation(o, this.JsonOutputContext.WritingResponse);
                ODataJsonValidationUtils.ValidateOperation(this.MetadataDocumentBaseUri, o);
        ...
        private string GetOperationMetadataString(ODataOperation operation)
        {
            ...
            Debug.Assert(ODataJsonUtils.IsMetadataReferenceProperty(operationMetadataString), "ODataJsonUtils.IsMetadataReferenceProperty(operationMetadataString)");
            ...
            Debug.Assert(
                !ODataJsonValidationUtils.IsOpenMetadataReferencePropertyName(this.MetadataDocumentBaseUri, operationMetadataString),
                "Open metadata reference property is not supported, we should have thrown before this point.");

            return ODataConstants.ContextUriFragmentIndicator + ODataJsonUtils.GetUriFragmentFromMetadataReferencePropertyName(this.MetadataDocumentBaseUri, operationMetadataString);
        }
        ...
        private void WriteOperationMetadataGroup(IGrouping<string, ODataOperation> operations)
        {
            ...
            this.JsonOutputContext.JsonWriter.WriteName(operations.Key);
        ...
        private void WriteOperation(ODataOperation operation)
        {
            ...
            {
                this.JsonOutputContext.JsonWriter.WriteName(JsonConstants.ODataOperationTitleName);
                this.JsonOutputContext.JsonWriter.WriteValue(operation.Title);
            }

            if (operation.Target != null)
            {
                string targetUrlString = this.GetOperationTargetUriString(operation);
                this.JsonOutputContext.JsonWriter.WriteName(JsonConstants.ODataOperationTargetName);
                this.JsonOutputContext.JsonWriter.WriteValue(targetUrlString);
            }
        ...
        private async Task WriteOperationMetadataGroupAsync(IGrouping<string, ODataOperation> operations)
        {
            ...
            if (useArray)
            {
                await this.JsonOutputContext.JsonWriter.StartArrayScopeAsync()
                    .ConfigureAwait(false);
            }
            ...
            if (useArray)
            {
                await this.JsonOutputContext.JsonWriter.EndArrayScopeAsync()
                    .ConfigureAwait(false);
            }
        }
        ...
        private async Task WriteOperationAsync(ODataOperation operation)
        {
            ...
            if (operation.Title != null)
            {
                await this.JsonOutputContext.JsonWriter.WriteNameAsync(JsonConstants.ODataOperationTitleName)
                    .ConfigureAwait(false);
                await this.JsonOutputContext.JsonWriter.WriteValueAsync(operation.Title)
                    .ConfigureAwait(false);
            }
            ...
            {
                string targetUrlString = this.GetOperationTargetUriString(operation);
                await this.JsonOutputContext.JsonWriter.WriteNameAsync(JsonConstants.ODataOperationTargetName)
                    .ConfigureAwait(false);
                await this.JsonOutputContext.JsonWriter.WriteValueAsync(targetUrlString)
                    .ConfigureAwait(false);
            }
