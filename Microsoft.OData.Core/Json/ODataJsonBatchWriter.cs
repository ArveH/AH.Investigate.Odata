...
namespace Microsoft.OData.Json
{
    ...
    /// </summary>
    internal sealed class ODataJsonBatchWriter : ODataBatchWriter
    {
        ...
        {
            get { return this.OutputContext as ODataJsonOutputContext; }
        }
        ...
        private ODataBatchOperationMessage CurrentOperationMessage
        {
            get
            {
                ...
                {
                    Debug.Assert(!this.JsonOutputContext.WritingResponse, "Request message can only be set when writing request.");
                    return this.CurrentOperationRequestMessage.OperationMessage;
                }
                else if (this.CurrentOperationResponseMessage != null)
                {
                    Debug.Assert(this.JsonOutputContext.WritingResponse, "Response message can only be set when writing response.");
        ...
        public override void StreamRequested()
        {
            ...
            this.JsonOutputContext.FlushBuffers();
        ...
        public override async Task StreamRequestedAsync()
        {
            ...
            await this.JsonOutputContext.FlushBuffersAsync()
                .ConfigureAwait(false);
            this.SetState(BatchWriterState.OperationStreamRequested);
        }
        ...
        Uri uri, string contentId, BatchPayloadUriOption payloadUriOption, IEnumerable<string> dependsOnIds)
        {
            ...
            this.CurrentOperationRequestMessage = BuildOperationRequestMessage(
                this.JsonOutputContext.GetOutputStream(), method, uri, contentId,
        ...
        protected override ODataBatchOperationResponseMessage CreateOperationResponseMessageImplementation(string contentId)
        {
            ...
            this.CurrentOperationResponseMessage = BuildOperationResponseMessage(
                this.JsonOutputContext.GetOutputStream(),
        ...
        private void WriteBatchEnvelope()
        {
            ...
            this.jsonWriter.WriteName(this.JsonOutputContext.WritingResponse ? PropertyResponses : PropertyRequests);
            this.jsonWriter.StartArrayScope();
        }
        ...
        private async Task WriteBatchEnvelopeAsync()
        {
            ...
            // Start the requests / responses property
            await this.jsonWriter.WriteNameAsync(this.JsonOutputContext.WritingResponse ? PropertyResponses : PropertyRequests)
                .ConfigureAwait(false);
            await this.jsonWriter.StartArrayScopeAsync()
                .ConfigureAwait(false);
        }
